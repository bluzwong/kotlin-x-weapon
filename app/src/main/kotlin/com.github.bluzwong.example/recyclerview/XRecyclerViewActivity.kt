...
public class XRecyclerViewActivity : AppCompatActivity() {
    val list:MutableList<MainListItem> = ArrayList()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = MainAdapter(layoutInflater)
        adapter!!.items = list
        val main_recycler_view = findViewById(R.id.main_recycler_view) as XRecyclerView
        val endlessAdapter = EndlessAdapterWrapper(main_recycler_view, adapter!!)
        endlessAdapter.setOnLoadListener {
            runDelayed(2222) {
                if (list.count() > 20) {
                    endlessAdapter.setStatusNoMore()
                    return@runDelayed
                }
                for (item in 0..2) {
                    list add MainListItem(R.layout.listitem_main_activity, "test $item" + if (item % 3 == 0) "\n" else "")
